...
class MinkShape {
        ...
        for (let i = 0; i < poly.points.length; i++) {  
            ...
            if (i > 0) {
                ...
                if (i == poly.points.length - 1) {
                    arcPairs.push([arcpoint2, pointlist[0]]);
                }
            }
        }
    ...
    display() {
        ...
        for(let i = 0; i < basePoints.length; i++) {
            stroke(0,0,255);
            circle(basePoints[i].x, basePoints[i].y, 5);
            stroke(255,0,0);
            circle(ArcPairs[i][0].x, ArcPairs[i][0].y, 5);
            circle(ArcPairs[i][1].x, ArcPairs[i][1].y, 5);

            drawArcCircle(basePoints[i].x, basePoints[i].y, ArcPairs[i][0].x, ArcPairs[i][0].y, ArcPairs[i][1].x, ArcPairs[i][1].y, Srad);
            // drawArcCircle(p0.x, p0.y, new_poly.points[(2*i)-1].x, new_poly.points[(2*i)-1].y, new_poly.points[(2*i)].x, new_poly.points[(2*i)].y, Srad);
        }
        endShape();
    }
}
...
function drawArcCircle(mx, my, x1, y1, x2, y2, radius) {  
    ...
    stroke(0);
    strokeWeight(2);
    arc(mx, my, radius * 2, radius * 2, endAngle, startAngle);
  }
